    ...
    <Layout>
      ...
      <LazyMotion features={domAnimation}>
        ...
        >
          <div className="p-4 lg:p-8 lg:absolute top-0 left-0 right-0 w-full">
            <div className="grid grid-cols-2 lg:grid-cols-12 gap-4 lg:gap-8 pt-12 lg:pt-0">
              <div className="col-span-2 lg:col-start-3 block">
                <TextReveal delay={0.1} className="leading-none">Words</TextReveal>
              </div>


              <div className="col-span-3 block leading-[0.9] text-gray">
                <TextReveal delay={0.2} className="leading-none text-black dark:text-white">All</TextReveal>
                {cats.map((e,i) => {
                  return (
                    <TextReveal delay={0.2} className="block relative overflow-hidden" key={i}>
                      <Link className="hover:text-black dark:hover:text-white transition-colors ease-[cubic-bezier(0.71,0,0.17,1)] duration-[500ms]" href={`/words/categories/${e.slug.current}`}>{e.title}</Link>
                    </TextReveal>
                  )
                })}
              </div>
          ...
          {/* Fixed Header Reveal */}
          <div className={`p-4 lg:p-8 fixed top-0 left-0 right-0 w-full z-[100] transition-transform ease-[cubic-bezier(0.71,0,0.17,1)] duration-[500ms] ${headerShown ? 'translate-y-0' : '-translate-y-full pointer-events-none' } bg-white dark:bg-black`}>
